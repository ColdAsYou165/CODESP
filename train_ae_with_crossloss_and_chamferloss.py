...
# parser.add_argument('--lr_d', type=float, default=0.0002, help='crossentropyloss lr, default=0.0002')
...
results_root = "../results/train_ae_with_crossloss_and_chamferloss_v1" + f"/{args}"
...
def ae(epoch):
    ...
    for batch_idx, (data, label) in enumerate(trainloader):
        ...
        data_concat = data_concat.reshape(-1, data_concat.shape[1] * 2, data_concat.shape[2], data_concat.shape[3])  # n/2 2h w c
    ...
    save_image(virtual_data, results_pic_root + f"/virpic_--epoch{epoch}--chamferloss{loss_chamfer_all:.3f}--crossloss{loss_cross_all:.3f}.jpg")

    # 保存模型权重
    if (epoch + 1) % 100 == 0 and True:
